...
function SelectImageStep({ onNext }: SelectImageStepProps) {
  ...
  const partitionCustomImages = customImages.filter(
    (img) => img.Compatibility.Partition,
  );

  const hasCachedPartitionImage =
    !loading &&
    (partitionCustomImages.length > 0 ||
  ...
  return (
    <div className="flash-step-panel">
      ...
      ) : partitionEntries.length === 0 && partitionCustomImages.length === 0 ? (
    ...
    try {
      const p =
        selectedImage.kind === "custom"
          ? await planCustomImagePartitionFlash(
              target.device.Path,
              target.partition.Path,
              selectedImage.image.ID,
            )
          : await planCataloguePartitionFlash(
              target.device.Path,
              target.partition.Path,
